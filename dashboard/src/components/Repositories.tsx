...
export default function Repositories({ onView }: { onView: (repository: string) => void }) {
  ...
  return <main className="agents-workspace">
    ...
    <div className="repositories-ledger" role="table" aria-label="Connected repositories">
      ...
      {!facets.isPending && !failed && repositories.length === 0 && <div className="state-message"><strong>No repositories connected yet</strong><span>Install the Coaudit GitHub App to start the audit trail, no CLI, no config.</span><a className="primary-button" href="#/connect">Connect GitHub</a></div>}
